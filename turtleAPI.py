...
class robot():
  ...
  def __init__(self):
    ...
    #node handeling occupancy Grid
    self.mapNode = rospy.Subscriber("/map",OccupancyGrid,self.__mapHandle)
  ...
  def printMap(self):
    ...
    # mapid:	contains the occupancy grid for user
    try:
      mapid = self.getMap()
      print(mapid.info)

      #additional data can be accessed with
      #print(mapid.info.resolution)
      #print(mapid.info.width)
      #print(mapid.info.height
      #print(mapid.info.origin) <-- this accesses into the mose data type google geomety_msg pose for information on sub information
      print(mapid.data) # contains the array of data usually 1-100 for occupancy and -1 for no idea.
    except Exception,e:
      print(str(e))
  ...
  def getAngle(self):
    ...
    try:
      #print(self.odom.pose.orientation)
      #print(self.odom.pose.position)
